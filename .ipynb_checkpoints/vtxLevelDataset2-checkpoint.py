...
def superbatch_iterator(files, keys, superbatch_size=1024*100):
    ...
    arrays = []
    num_entries = 0
    iterator = uproot.iterate(files, keys, step_size=superbatch_size)
...
def pad_and_fill(X, branchDict, svDim=12, tkDim=10, fillValue=-9e10):
    def process_field(field, to_broadcast=False):
        if to_broadcast:
            assert 'SDVSecVtx_Lxy' in branchDict['sv'], 'Lxy must be in the secondary vertex branches.'
            broadcasted = ak.broadcast_arrays(X[field], X['SDVSecVtx_Lxy'])[0]
            flat = ak.flatten(broadcasted, axis=1)
        else:
            flat = ak.flatten(X[field], axis=1)
